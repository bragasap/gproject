...
func _input(event: InputEvent) -> void:
	if event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_RIGHT:
		...
		#work here to update per unit behavior
		formation(get_tree().get_nodes_in_group("selected-units"),result["position"])
...
##I dont think I need to fix this anymore will figure it out later
func selectUnits():
...
func formation(units:Array[Node],target:Vector3):
		#var x = 0
		#var y = 0
		#var spacing =1
		for unit in units:
			unit.newWaypoint(target)
		#x+=spacing
